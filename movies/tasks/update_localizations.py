...
class UpdateLocalizationsTask(Task):
    name = 'movies.tasks.update_localizations'

    def run(self):
        logger.info("Beginning update")
        n = 0
        for n, item in enumerate(self.get_movie_items()):
    ...
    def update_item(self, item):
        ...
        language_list = self.get_languages(item)
        for imdb_id, language in product(imdb_id_list, language_list):
            self.update_item_localization(item, imdb_id, language)
    ...
    @transaction.atomic
    def update_item_localization(self, item, imdb_id, language):
        localization, created = Localization.objects.update_or_create(
            movie_id=imdb_id, language=language,
            defaults=self.get_localization_data(item, language))

        poster_source = self.get_poster_source(
            localization.wikipedia_page, language)
        if poster_source:
    ...
    def get_localization_data(self, item, language):
        ...
        title = self.clean_title(
            self.choose_title(wikipedia_page, label, language))
    ...
    def choose_title(self, wikipedia_page, label, language):
        """
    ...
    def get_poster_source(self, page_title, language):
        """
        ...
        filename = self._find_poster_filename(movie_page)
        if not filename:
            return None
        ...
        file_url = (
            'https://upload.wikimedia.org/wikipedia/{0}/{1}/{2}/{3}'.format(
                language, hashed_filename[0], hashed_filename[:2],
                quote(sane_filename)))

        return file_url, file_updated

    def _find_poster_filename(self, page):
        """
