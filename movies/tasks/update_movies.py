...
class UpdateMoviesTask(Task):
    name = 'movies.tasks.update_movies'
    ...
    def update_item(self, item):
        movie, created = Movie.objects.update_or_create(
            imdb_id=item.imdb_id, defaults=self.item_to_updated_fields(item))
    ...
    def item_to_updated_fields(self, item):
        return {
            'title': item.title,
            'year': item.year,
